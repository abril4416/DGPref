...
def mine_maybe_log_save_evaluate(trainer,all_iters, all_acc, all_auc, logger, opt,processor,
    ...
    if trainer.control.should_evaluate:
        ...
        if trainer.state.global_step>=opt.gen_start and trainer.state.global_step%opt.gen_step==0 and opt.DATA_NAME in ['mem','harm','mimc','few-mem','few-harm','few-mimc']:
        ...
        elif trainer.state.global_step>=opt.gen_start and trainer.state.global_step%opt.gen_step==0 and opt.DATA_NAME in ['hate-speech']:
            invalid,auc,acc=hsp_generation(model,processor,opt)
            if len(invalid)>0:
                logger.write('\tThere are %d invalid examples' % (len(invalid)))
            logger.write('\tAUC %.2f, Acc %.2f' % (auc,acc))
            all_acc.append(acc)
...
def mine_maybe_log_save_evaluate(trainer,all_iters, all_acc, all_auc, logger, opt, processor,
    ...
    if trainer.control.should_evaluate:
        ...
        if trainer.state.global_step>=opt.gen_start and trainer.state.global_step%opt.gen_step==0 and opt.DATA_NAME in ['mem','harm','mimc']:
            invalid,auc,acc=hfm_generation(model,processor,opt)
            if len(invalid)>0:
                logger.write('\tThere are %d invalid examples' % (len(invalid)))
            logger.write('\tAUC %.2f, Acc %.2f' % (auc,acc))
...
def rewrite_train(trainer,processor,opt,logger,trail=None,ignore_keys_for_eval=None):
    ...
    for epoch in range(epochs_trained, num_train_epochs):
        ...
        for step, inputs in enumerate(epoch_iterator):
            ...
            with trainer.accelerator.accumulate(model):
                ...
                print (inputs['pixel_values'].dtype,model.dtype)
                print (inputs['input_ids'].dtype,inputs['labels'].dtype)
                """
            ...
            if (
                ...
                mine_maybe_log_save_evaluate(trainer,all_iters,all_acc, all_auc,logger,opt,processor,
                                             tr_loss, model, trial, epoch, ignore_keys_for_eval)
            else:
...
def train_for_epochs(model,processor,args,
                     data_collator,
                     train_set,test_set):
    gradient_accumulation_steps = args.batch_size // args.micro_batch_size
    #logger initialization
    log_path=os.path.join(args.DATA_NAME)
